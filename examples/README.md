...
npm run pack:all
cd examples/vanilla
npm install ../../artifacts/packages/quilt-core-0.2.3.tgz ../../artifacts/packages/quilt-vanilla-0.2.3.tgz
npm run dev
```

Use the same procedure in `examples/electron`, then `npm start`. For
`examples/react`, include `../../artifacts/packages/quilt-react-0.2.3.tgz` in
